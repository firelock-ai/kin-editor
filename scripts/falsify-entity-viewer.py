...
MUTATIONS: list[Mutation] = [
    ...
    Mutation(
        ...
        old="  if (queryValues(parts.query, ENTITY_ID_PARAM).length !== 1) return undefined;\n  const entityId = readQueryParam(parts.query, ENTITY_ID_PARAM);\n  if (!entityId) {\n    return undefined;\n  }",
        new="  const entityId = readQueryParam(parts.query, ENTITY_ID_PARAM);\n  if (!entityId) {\n    return { workspaceKey: parts.authority, entityId: parts.path };\n  }",
    ...
    Mutation(
        name="a draft save is acknowledged before the daemon replies",
        path="src/entity-draft-session.ts",
        old="      const saved = await this.transport().saveDraft(request);",
        new="      const saved = { ...this.draft, body };",
        suite="src/__tests__/entity-draft-session.test.ts",
        guards="Save acknowledges only daemon-persisted text",
    ),
    Mutation(
